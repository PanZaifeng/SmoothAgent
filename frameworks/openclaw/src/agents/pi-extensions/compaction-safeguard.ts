...
import {
  BASE_CHUNK_RATIO,
  type CompactionSummaryExtraOptions,
...
import {
  clearCompactionLookahead,
  type CompactionLookaheadCandidate,
  getCompactionLookaheadCandidate,
...
type SglangRequestClass = "fg" | "bg";

type SglangRequestMetadataOptions = {
  lookaheadGroupId?: string;
  onPayload?: CompactionSummaryExtraOptions["onPayload"];
};

function applySglangRequestMetadata(
  payload: unknown,
  requestClass: SglangRequestClass,
  options?: SglangRequestMetadataOptions,
): void {
  if (!payload || typeof payload !== "object") {
    return;
  }
  const payloadObj = payload as Record<string, unknown>;
  payloadObj.request_class = requestClass;
  payloadObj.priority_class = requestClass === "bg" ? "be" : "lc";
  payloadObj.is_lookahead = requestClass === "bg";
  if (options?.lookaheadGroupId) {
    payloadObj.lookahead_group_id = options.lookaheadGroupId;
  }
}

export function createSglangRequestClassExtraOptions(
  requestClass: SglangRequestClass,
  options?: SglangRequestMetadataOptions,
): CompactionSummaryExtraOptions {
  return {
    onPayload: (payload, payloadModel) => {
      applySglangRequestMetadata(payload, requestClass, options);
...
export type CompactionSummaryBuildResult = {
  summary: string;
  firstKeptEntryId?: string;
  tokensBefore: number;
  details: {
 ...
 */
async function readWorkspaceContextForSummary(workspaceDir = process.cwd()): Promise<string> {
...
function logCompactionSummaryEvent(params: {
  event:
    | "lookahead_hit"
    | "lookahead_miss_not_found"
    | "lookahead_miss_in_flight"
    | "lookahead_miss_stale"
    | "compaction_summary_live_start"
    | "compaction_summary_live_end";
  sessionId?: string;
...
function isLookaheadCandidateStaleForPreparation(
  candidate: CompactionLookaheadCandidate,
  preparation: CompactionPreparationLike,
): boolean {
  if (candidate.status !== "ready") {
    return false;
  }
  const candidateFirstKeptEntryId = candidate.firstKeptEntryId;
  const preparationFirstKeptEntryId = preparation.firstKeptEntryId;
  if (candidateFirstKeptEntryId === preparationFirstKeptEntryId) {
    return false;
  }
  return Boolean(candidateFirstKeptEntryId || preparationFirstKeptEntryId);
}

function isLookaheadCandidateUsable(
  candidate: CompactionLookaheadCandidate | undefined,
  preparation: CompactionPreparationLike,
): candidate is CompactionLookaheadCandidate & { status: "ready"; summary: string } {
  return Boolean(
    candidate?.status === "ready" &&
      candidate.summary &&
      !isLookaheadCandidateStaleForPreparation(candidate, preparation),
  );
}

function hasRealCompactionConversation(preparation: CompactionPreparationLike): boolean {
  ...
  const hasRealTurnPrefix = turnPrefixMessages.some((message, index, messages) =>
    isRealConversationMessage(message, messages, index),
  );
  return hasRealSummarizable || hasRealTurnPrefix;
}

export async function buildCompactionSummaryFromPreparation(params: {
  preparation: CompactionPreparationLike;
  customInstructions?: string;
  signal: AbortSignal;
  model: NonNullable<ExtensionContext["model"]>;
  apiKey: string;
  runtime?: CompactionSafeguardRuntimeValue;
  workspaceDir?: string;
  sessionId?: string;
  trigger?: "soft" | "hard";
  extraOptions?: CompactionSummaryExtraOptions;
}): Promise<CompactionSummaryBuildResult> {
  const { preparation, customInstructions, signal, apiKey } = params;
  ...
  const trigger = params.trigger ?? "hard";
  const buildStartedAt = Date.now();

  if (trigger === "hard") {
    logCompactionSummaryEvent({
      event: "compaction_summary_live_start",
      sessionId: params.sessionId,
      model,
      trigger,
  ...
  const workspaceContext = await readWorkspaceContextForSummary(params.workspaceDir);
  if (workspaceContext) {
  ...
  if (trigger === "hard") {
    logCompactionSummaryEvent({
      event: "compaction_summary_live_end",
      sessionId: params.sessionId,
      model,
      trigger,
...
export default function compactionSafeguardExtension(api: ExtensionAPI): void {
  // Type assertion for api.on overload resolution (session_before_compact exists on ExtensionAPI).
  (api.on as (event: "session_before_compact", handler: (event: unknown, ctx: ExtensionContext) => Promise<unknown>) => void)(
    "session_before_compact",
    async (event, ctx) => {
    const { preparation, customInstructions, signal } = event as SessionBeforeCompactEvent;
    const sessionId = ctx.sessionManager.getSessionId?.();
    if (!hasRealCompactionConversation(preparation)) {
      log.info(
        "Compaction safeguard: no real conversation messages to summarize; writing compaction boundary.",
      );
      return {
        compaction: {
    ...
    const lookaheadCandidate = sessionId ? getCompactionLookaheadCandidate(sessionId) : undefined;
    if (isLookaheadCandidateUsable(lookaheadCandidate, preparation)) {
      logCompactionSummaryEvent({
        event: "lookahead_hit",
        sessionId,
        model,
        trigger: "hard",
        tokensBefore: lookaheadCandidate.tokensBefore,
        usedCachedSummary: true,
      });
      if (sessionId) {
        clearCompactionLookahead(sessionId);
      }
      return {
        compaction: {
          summary: lookaheadCandidate.summary,
          firstKeptEntryId:
            lookaheadCandidate.firstKeptEntryId ?? preparation.firstKeptEntryId,
          tokensBefore: preparation.tokensBefore ?? lookaheadCandidate.tokensBefore ?? 0,
          details: lookaheadCandidate.details,
        },
      };
    }

    const lookaheadMissEvent =
      lookaheadCandidate?.status === "in_flight"
        ? "lookahead_miss_in_flight"
        : lookaheadCandidate &&
            isLookaheadCandidateStaleForPreparation(lookaheadCandidate, preparation)
          ? "lookahead_miss_stale"
          : "lookahead_miss_not_found";

    if (lookaheadMissEvent === "lookahead_miss_stale" && sessionId) {
      clearCompactionLookahead(sessionId);
    }

    logCompactionSummaryEvent({
      event: lookaheadMissEvent,
      sessionId,
      model,
      trigger: "hard",
      tokensBefore: preparation.tokensBefore,
    ...
    try {
      const compaction = await buildCompactionSummaryFromPreparation({
        preparation,
        customInstructions,
        signal,
        model,
        ...
        trigger: "hard",
        extraOptions:
          runtime?.sglangSchedulingEnabled && model.api === "openai-completions"
            ? createSglangRequestClassExtraOptions("fg")
            : undefined,
      });
...
export const __testing = {
  ...
  auditSummaryQuality,
  hasRealCompactionConversation,
  hasMeaningfulConversationContent,
  isRealConversationMessage,
  isLookaheadCandidateStaleForPreparation,
  isLookaheadCandidateUsable,
  computeAdaptiveChunkRatio,
  isOversizedForSummary,
