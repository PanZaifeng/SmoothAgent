...
import { extractAssistantTextForPhase as extractChatAssistantTextForPhase } from "../shared/chat-message-content.js";
...
function finalizeAssistantText(msg: AssistantMessage, extracted: string): string {
...
export function extractAssistantVisibleText(msg: AssistantMessage): string {
  const finalAnswerText = extractChatAssistantTextForPhase(msg, {
    phase: "final_answer",
    sanitizeText: sanitizeAssistantText,
    joinWith: "\n",
  });
  if (finalAnswerText?.trim()) {
    return finalizeAssistantText(msg, finalAnswerText);
  }

  const visibleText = extractChatAssistantTextForPhase(msg, {
    sanitizeText: sanitizeAssistantText,
    joinWith: "\n",
  });
  return finalizeAssistantText(msg, visibleText ?? "");
}
...
export function extractAssistantText(msg: AssistantMessage): string {
  ...
  return finalizeAssistantText(msg, extracted);
}
