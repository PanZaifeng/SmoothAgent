...
import { resolveSglangSchedulingEnabled } from "./model.js";
import { createStreamingUsageBackfillWrapper } from "./usage-stream-backfill.js";
...
function createStreamFnWithExtraParams(
  ...
  if (resolveSupportedTransport(transport)) {
...
function createSglangForegroundRequestMetadataWrapper(
  baseStreamFn: StreamFn | undefined,
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    ...
    return underlying(model, context, {
      ...
      onPayload: (payload, payloadModel) => {
        if (payload && typeof payload === "object") {
          ...
          payloadObj.request_class = "fg";
          payloadObj.priority_class = "lc";
          payloadObj.is_lookahead = false;
        }
        return originalOnPayload?.(payload, payloadModel);
      },
    });
...
export function applyExtraParamsToAgent(
  ...
  if (resolveSglangSchedulingEnabled({ cfg, providerId: provider })) {
    log.debug(`applying SGLang foreground request metadata for ${provider}/${modelId}`);
    agent.streamFn = createSglangForegroundRequestMetadataWrapper(agent.streamFn);
  }

  // SGLang and some OpenAI-compatible servers omit streaming `usage`; Pi then keeps zeros and
  // never reaches threshold compaction. Estimate from messages when usage is still all-zero.
  agent.streamFn = createStreamingUsageBackfillWrapper(agent.streamFn);
