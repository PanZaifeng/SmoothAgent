...
type PrepareCompactCapableSession = {
  messages: AgentMessage[];
  agent: {
    replaceMessages: (messages: AgentMessage[]) => void;
  };
  prepareCompact: (customInstructions?: string) => Promise<{
...
export type EmbeddedPiPreparedCompactionResult =
  | {
      ok: true;
      provider: string;
      modelId: string;
      ...
      apiKey: string;
      workspaceDir: string;
      preparation: Awaited<ReturnType<PrepareCompactCapableSession["prepareCompact"]>>;
    }
  | {
      ok: false;
      reason: string;
...
export async function prepareEmbeddedPiSessionCompaction(
  params: CompactEmbeddedPiSessionParams,
): Promise<EmbeddedPiPreparedCompactionResult> {
  const resolvedWorkspace = resolveUserPath(params.workspaceDir);
  ensureRuntimePluginsLoaded({
    config: params.config,
    workspaceDir: resolvedWorkspace,
  ...
  const fail = (reason: string): EmbeddedPiPreparedCompactionResult => ({
    ok: false,
    reason,
  });
  ...
  if (!model) {
    return fail(error ?? `Unknown model: ${provider}/${modelId}`);
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (!session.messages.some(hasRealConversationContent)) {
          return fail("no real conversation messages");
        }

        const prepareCompact = (session as Partial<PrepareCompactCapableSession>).prepareCompact;
        if (typeof prepareCompact !== "function") {
          return fail(
            "session.prepareCompact() is unavailable; apply the @mariozechner/pi-coding-agent prepareCompact patch first.",
          );
        }
        const prepared = await prepareCompact.call(session, params.customInstructions);
        return {
          ok: true,
          provider,
          modelId,
