...
function mergeConfiguredProviderRequest(params: {
  providerConfig?: InlineProviderConfig;
  configuredModel?: { request?: ModelProviderRequestTransportOverrides };
}): ModelProviderRequestTransportOverrides | undefined {
  const providerRequest = params.providerConfig?.request;
  const modelRequest = params.configuredModel?.request;
  if (!providerRequest && !modelRequest) {
    return undefined;
  }
  return {
    ...providerRequest,
    ...modelRequest,
    headers:
      providerRequest?.headers || modelRequest?.headers
        ? {
...
function attachConfiguredProviderRequest<TModel extends Model<Api>>(params: {
  model: TModel;
  providerConfig?: InlineProviderConfig;
  configuredModel?: { request?: ModelProviderRequestTransportOverrides };
}): TModel {
  return attachModelProviderRequestTransport(
    params.model,
    mergeConfiguredProviderRequest({
...
export function resolveSglangSchedulingEnabled(params: {
  cfg?: OpenClawConfig;
  providerId?: string;
}): boolean {
  ...
  const providerConfig = resolveConfiguredProviderConfig(params.cfg, providerId);
  return providerConfig?.sglangScheduling ?? false;
}

export function shouldInjectSglangRequestClass(params: {
  ...
  const providerId =
    params.providerId?.trim() || (typeof params.model.provider === "string" ? params.model.provider : "");
  return resolveSglangSchedulingEnabled({
    cfg: params.cfg,
    providerId,
  });
}
...
function applyConfiguredProviderOverrides(params: {
  ...
  return attachConfiguredProviderRequest({
    model: {
      ...discoveredModel,
...
export function buildInlineProviderModels(
  ...
  return Object.entries(providers).flatMap(([providerId, entry]) => {
    ...
    return (entry?.models ?? []).map((model) =>
      attachConfiguredProviderRequest({
        model: {
          ...model,
          provider: trimmed,
        ...
        configuredModel: model as { request?: ModelProviderRequestTransportOverrides },
      }),
    );
  });
}
...
export function resolveModelWithRegistry(params: {
  ...
  if (providerConfig || modelId.startsWith("mock-")) {
    return normalizeResolvedModel({
      provider,
      model: attachConfiguredProviderRequest({
        model: {
          id: modelId,
          name: modelId,
        ...
        providerConfig,
        configuredModel: configuredModel as
          | { request?: ModelProviderRequestTransportOverrides }
          | undefined,
      }),
    });
